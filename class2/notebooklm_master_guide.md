...
* **🖥️ AI Unified Gateway ─ NotebookLM 專區門戶平台介面**：
  ...
  | **Project4_Skyline企業知識大腦分布索引與標案指南** | `2c039250-b2a8-4cf1-bc3d-f0f8b8441dc3` | `NOTEBOOKLM_SYNC` | **大腦四**：導航索引大腦，整合共享大腦分布索引底稿 (Section 9)。 |
  | **Project5_SkyLine專案執行經驗與改善對策庫** | `73085c64-dea5-4945-9226-949023b0ac9b` | `NOTEBOOKLM_SYNC` | **大腦五**：歷史備份與改善對策庫，作為對照與補充檢索。 |
...
為了讓同仁與學員在導入這五本「企業共享大腦」時能即裝即用，以下設計了多組實務高價值、符合人機協同心智的黃金 Prompt 範本，可直接複製並貼入 NotebookLM 中提問：
...
### 🧠 大腦三：Project3_專案執行經驗對策大腦

#### 📝 提示詞範本 ─ 2 組

##### 📝 提示詞一：外協信用交叉比對與合約合規條款升級 (Complex Vendor Contract Enhancement)
* **設計目的**：深度稽核歷史履約缺失並自動輸出可寫入合約的 SLA KPI 考核細則。
* **推薦 Prompt 範本**：
  ```text
  你現在是 Skyline 採購合規與法務總監。請全面檢索歷史 Lessons Learned 數據，針對【名音】這家協力廠商在過去專案中的履約缺失進行深度比對：
  1. 統計其最常發生的缺失類別（如設備跳電、工程遲到、配合度差）與發生頻率。
  2. 提取出過去 PM 曾使用的補救方案，並評估其成效。
  3. 為了防範未來標案重複踩坑，請為我擬定 3 條具備法律約束力、且與 SLA 掛鉤的合約考核指標（KPI）及罰扣細則。
  ```

##### 📝 提示詞二：年度活動現場防禦系統與 PM 培訓防坑點檢表 (Complex Pitfalls Summarization & Training Guide)
* **設計目的**：系統性歸納 36 筆原始反饋並轉譯為 PM 培訓現場避坑指南。
* **推薦 Prompt 範本**：
  ```text
  你現在是 Skyline 企業品質保證 (QA) 總監。請通讀所有專案回報紀錄，為新進 PM 團隊建立一套專案安全防禦與培訓系統：
  1. 歸納出歷史專案中最常發生的前 3 大現場缺失，並分析其具體成因。
  2. 對於每一類缺失，從歷史檔案中提取出已驗證有效的「最佳實踐解決對策」。
  3. 整合這些經驗，為新進 PM 製作一份條列式的「活動現場安全與行政防坑檢核表 (Lessons Learned Checklist)」。
  ```

---

### 🧠 大腦四：Project4_Skyline企業知識大腦分布索引與標案指南

#### 📝 提示詞範本 ─ 2 組

##### 📝 提示詞一：工作痛點實務導航 (Scenario Router)
* **設計目的**：根據工作遇到的具體痛點，推薦應開啟的大腦專案與對應的檔案 and 推薦關鍵字。
* **推薦 Prompt 範本**：
  ```text
...
* **設計目的**：幫忙定位特定業務知識（如廠商實績、SLA 限制或備降機制）儲存於哪本大腦專案與檔案。
* **推薦 Prompt 範本**：
  ```text
...
### 🧠 大腦五：Project5_SkyLine專案執行經驗與改善對策庫

#### ✍️ 簡單版本 (Simple Prompts) ─ 3 組

##### 📝 提示詞一：寵物嘉年華現場意外防坑檢索 (Pet Event Exploratory Check)
* **設計目的**：以探索式問法引導大腦主動挖掘寵物活動的歷史安全與秩序缺失。
* **推薦 Prompt 範本**：
  ```text
  我們即將舉辦大型的寵物嘉年華，請幫我檢索歷史專案經驗，在寵物的安全、現場衛生或秩序管理上，曾經出過哪些意外狀況？強烈建議條列說明當時分別是如何處置的？
  ```

##### 📝 提示詞二：特定廠商履約缺失快速查核 (Vendor Quick Audit)
* **設計目的**：秒查特定外協廠商在專案中的歷史表現。
* **推薦 Prompt 範本**：
  ```text
  請檢索所有 PM 經驗回報，列出與【名音】（或特定外協廠商）相關的缺失紀錄與當時 PM 的解決對策。
  ```

##### 📝 提示詞三：財務核銷與行政防坑點檢 (Admin Pitfall Avoidance)
* **設計目的**：在專案啟動前快速稽核常見的核銷與行政文宣痛點。
* **推薦 Prompt 範本**：
  ```text
  請檢索歷史回報，歸納所有與「財務核銷」與「文宣印刷」相關的常見痛點與預防措施。
  ```

#### 🧠 複雜版本 (Complex Prompts) ─ 2 組

##### 📝 提示詞四：外協信用交叉比對與合約合規條款升級 (Complex Vendor Contract Enhancement)
* **設計目的**：深度稽核歷史履約缺失並自動輸出可寫入合約的 SLA KPI 考核細則。
* **推薦 Prompt 範本**：
  ```text
  你現在是 Skyline 採購合規與法務總監。請全面檢索歷史 Lessons Learned 數據，針對【名音】這家協力廠商在過去專案中的履約缺失進行深度比對：
...
##### 📝 提示詞五：年度活動現場防禦系統與 PM 培訓防坑點檢表 (Complex Pitfalls Summarization & Training Guide)
* **設計目的**：系統性歸納 36 筆原始反饋並轉譯為 PM 培訓現場避坑指南。
* **推薦 Prompt 範本**：
  ```text
  你現在是 Skyline 企業品質保證 (QA) 總監。請通讀所有專案回報紀錄，為新進 PM 團隊建立一套專案安全防禦與培訓系統：
...
## 💡 9. 大腦四的導覽與知識底稿 (Project4 SSOT Catalog Guide)
