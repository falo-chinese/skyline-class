...
## 💡 教材補充資訊：實務級多維模擬資料集設計 (Mock Dataset Specifications)

本補充資訊記錄了教材自小樣本驗證迭代至實務多維度場景的測試設計與規格。旨在提供學員一個具備「真實決策、合規判定與安全防護」的本地測試沙盒環境。

### 1. 模擬資料集檔案清冊
所有實體 PDF 與 Excel 檔案均由地端生成指令碼 `generate_mock_data.py` 動態產生，具體結構如下：

- **招標規範 (RFP)**:
  - `原始標案資源/招標需求/台南玩具展_RFP.pdf`：SLA 要求 < 2h；單案實績金額 >= 300 萬；加分項：綠建材佔比 > 30%。
  - `原始標案資源/招標需求/台北設計週_RFP.pdf`：SLA 要求 < 1h；單案實績金額 >= 500 萬。
- **協力廠商資歷與承諾 (Suppliers)**:
  - `原始標案資源/協力商大同/大同搭建商_實績證明.pdf`：高雄動漫節搭建案金額 NT$ 3,500,000 (符合玩具展，但不符設計週)；內含負責人林大同身分證字號 `A123456789` 與手機 `0912-345-678`。
  - `原始標案資源/協力商大同/大同搭建商_環保建材認證.pdf`：環保建材佔比達 35% (滿足玩具展加分項)。
  - `原始標案資源/協力商華麗/華麗設計商_實績證明.pdf`：台北文創展搭建案金額 NT$ 2,800,000 (低於 300 萬，觸發廢標風險)。
  - `原始標案資源/協力商名音/名音燈光商_SLA承諾.pdf`：現場響應 SLA 為 4 小時 (不合規，違反玩具展 2h 限制)；技術報價 NT$ 800,000。
  - `原始標案資源/協力商極光/極光音響商_SLA承諾.pdf`：現場響應 SLA 為 2 小時 (完全合規)；技術報價 NT$ 1,200,000。
- **公司證照庫 (PMP Certificates)**:
  - `原始標案資源/公司證照庫/王志明_Jimmy_PMP_2026.pdf`：王志明之 PMP 證書，到期日 2026-02-15 (投標時已過期 🚨)。
  - `原始標案資源/公司證照庫/林淑芬_Sophia_PMP_2028.pdf`：林淑芬之 PMP 證書，到期日 2028-12-31 (有效 🟢)。
  - `原始標案資源/公司證照庫/陳俊宏_Alex_PMP_2025.pdf`：陳俊宏之 PMP 證書，到期日 2025-08-31 (已過期 🚨)。
  - `原始標案資源/公司證照庫/張雅婷_Tina_PMP_2029.pdf`：張雅婷之 PMP 證書，到期日 2029-05-31 (有效 🟢)。

### 2. 真實測試場景與稽核判決
AI PM 在執行一條龍對帳時，會基於上述多維資料觸發以下實務場景：

1. **資安個資監控 (PII Scrubbing)**:
   - 稽核 `大同搭建商_實績證明.pdf` 時，AI 必須偵測並自動遮蔽身分證字號 `A123456789` 與電話 `0912-345-678`。
2. **SLA 響應缺口決策 (SLA Audit & Budgeting)**:
   - 對比玩具展 RFP 與名音燈光承諾時，發現其 4h SLA 違反 2h 限制。
   - 主管主控台會觸發預算加急按鈕，向名音燈光加購 2h SLA 服務（預算追加 30 萬），或建議主管直接更換為符合 2h SLA 且完全合規的 `極光音響商` (報價 120 萬)。
3. **PM 證照失效與備降調度 (PM Disqualification Fallback)**:
   - 系統比對資料庫發現預定 PM 志明之證書已於 2026-02-15 失效，主動建議以林淑芬 (Sophia) 替換之，並產生履歷異動日誌。
