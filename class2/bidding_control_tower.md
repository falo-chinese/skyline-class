...
1. **第一步：標前文件整理 (Intake & Excel Build)**：
   ...
   - **子環節**：1.1 複製資源至暫存區 ➔ 1.2 智慧語意檔名重置 ➔ 1.3 內容分析與類別歸類 ➔ 1.4.A 地端 Excel 試算表資料庫建置 (首次無範本自規劃建置) ➔ 1.4.B 地端 Excel 試算表資料庫建置 (依據參考範本對齊建置)。建立或更新本地 Excel 試算表資料庫 `標案工作暫存區/標案工作記錄表.xlsx`。
