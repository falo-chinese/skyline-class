...
1. **第一步：標前文件整理 (Intake & Excel Build)**：
   - **地端輸入**：`原始標案資源/招標需求/台南玩具展_RFP.pdf`、`原始標案資源/協力商大同/大同搭建商_實績證明.docx`、`原始標案資源/協力商名音/名音燈光商_SLA承諾.txt`、`原始標案資源/公司證照庫/林淑芬_Sophia_PMP_2028.pdf`。
   - **子環節**：1.1 複製資源至暫存區 ➔ 1.2 智慧語意檔名重置 ➔ 1.3 內容分析與類別歸類 ➔ 1.4 地端 Excel 試算表資料庫建置。建立本地 Excel 試算表資料庫 `標案工作暫存區/標案工作記錄表.xlsx`。
2. **第二步：招標合規差距對帳 (Compliance Gap Audit)**：
   - **地端輸入**：`標案工作暫存區/2026_招標需求_台南玩具展_RFP.txt` 及 Excel 試算表資料庫 `標案工作暫存區/標案工作記錄表.xlsx`。
   - **子環節**：2.1 廢標條款自動提取 ➔ 2.2 PM 證照過期稽核 ➔ 2.3 SLA 技術對帳比對。
3. **第三步：SLA 成本溢價與利潤決策 (SLA Surcharge & Costing)**：
   - **地端輸入**：Excel 試算表資料庫 `標案工作暫存區/標案工作記錄表.xlsx`。
...
4. **第四步：Delta 草稿自動組裝 (NotebookLM & Draft Compiling)**：
   - **地端輸入**：NotebookLM 知識庫、Excel 試算表資料庫 `標案工作暫存區/標案工作記錄表.xlsx`、草稿模板。
   - **子環節**：4.1 NotebookLM 真理檢索 ➔ 4.2 PM 備降人員替換（Sophia 取代志明） ➔ 4.3 標註人類審查點。
5. **第五步：實機跑測與一鍵收割 (Sandbox Run & Harvest)**：
   - **地端輸入**：`標案工作暫存區/draft_assembled.md`、Excel 試算表資料庫 `標案工作暫存區/標案工作記錄表.xlsx`。
   - **子環節**：5.1 Computer Use 網頁上傳模擬 ➔ 5.2 Excel SSOT 資料庫寫入 ➔ 5.3 知識大腦同步發布。
6. **第六步：AI PM 排程與進度協調 (Progress & AI PM Orchestration)**：
   - **地端輸入**：Excel 專案排程表、Staging 執行日誌。
   - **子環節**：6.1 AI PM 進度與風險監控 ➔ 6.2 協力商缺件語意催辦 ➔ 6.3 證照失效主動調配。
