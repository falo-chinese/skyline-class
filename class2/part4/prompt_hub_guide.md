...
*   💾 **下載 NotebookLM 黃金 Prompt 資源包 (CSV)**：[notebooklm_shared_brains_prompts.csv](notebooklm_shared_brains_prompts.csv) (相容 Prompt Hub，含 13 組大腦範本)
...
flowchart TD
    ...
    B -->|2. 載入本機 CSV 資源包| C[13 組 NotebookLM 黃金提示詞]
...
4. 匯入完成後，側邊欄會即時渲染出 `Project1_標案黃金範本大腦`、`Project2_協力商資歷與合規庫`、`Project3_專案執行經驗對策大腦`、`Skyline 企業知識大腦分布索引與標案指南`、`SkyLine 專案執行經驗與改善對策庫` 五個新分組，共 13 組黃金範本！
