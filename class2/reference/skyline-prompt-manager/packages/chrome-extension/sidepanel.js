// Skyline Prompt Manager - Satellite Chrome Extension JS
// Handles local storage, variables caching, DOM injection, CRUD, and Live Sync with PWA.

const STORAGE_KEYS = {
  database: "skyline_prompt_manager_database_v03",
  variables: "skyline_prompt_manager_variables_v03",
  theme: "skyline_prompt_manager_theme_v03",
  mode: "skyline_prompt_manager_ui_mode_v03",
  fontSize: "skyline_prompt_manager_font_size_v03"
};
...
const PWA_PATTERNS = [
  "*://localhost/*",
  "*://127.0.0.1/*",
  "*://__Skyline_TAIWAN_IO__/prompt-demo/*",
  "*://__Skyline_CHINESE__.github.io/prompt-demo/*"
];
...
async function scanPwaTabs() {
  ...
  for (const tab of tabs) {
    ...
    if (matched) {
      try {
        // Execute a quick script to check for Skyline PWA metadata in the MAIN world context
        const [res] = await chrome.scripting.executeScript({
          target: { tabId: tab.id },
          world: 'MAIN',
          func: () => {
            return typeof window.__Skyline_PWA_METADATA__ !== 'undefined' ? window.__Skyline_PWA_METADATA__ : null;
          }
        });
        if (res && res.result) {
          found.push({
            ...
            sourceName: res.result.sourceName || "Skyline PWA",
            url: tab.url
          });
        }
      } catch (e) {
  ...
  if (activePwaTabId) {
    try {
      chrome.scripting.executeScript({
        ...
        func: (extId) => {
          window.dispatchEvent(new CustomEvent("skylineExtensionPing", {
            detail: {
              extensionId: extId,
              clientName: "Skyline Satellite Client"
            }
          }));
        }
      });
...
async function pullFromPwa() {
  ...
  try {
    ...
    chrome.scripting.executeScript({
      ...
      func: () => {
        return {
          database: localStorage.getItem("skyline_prompt_manager_database_v03"),
          variables: localStorage.getItem("skyline_prompt_manager_variables_v03")
        };
      }
    }, async (results) => {
...
async function pushToPwa() {
  ...
  try {
    ...
    chrome.scripting.executeScript({
      ...
      args: [dbStr, varStr],
      func: (db, vars) => {
        localStorage.setItem("skyline_prompt_manager_database_v03", db);
        localStorage.setItem("skyline_prompt_manager_variables_v03", vars);
        // Dispatch custom event to notify page if it's listening
        window.dispatchEvent(new CustomEvent("skylineDbSync", { detail: { updated: true } }));
...
function exportCsv() {
  try {
    ...
    const filename = `skyline_prompt_export_extension_${stamp}.csv`;
