# Skyline Prompt Manager

Skyline Prompt Manager 是一個跨課程可重複使用的 Prompt 管理器主題單元。

## 版本備註

- 版本：v1.01
- 標記：Skyline x Force Cheng
- 發布日期：2026/6/1
...
在 Skyline 產品線中，本專案屬於 **Skyline Prompt**，也就是 Prompt Asset Layer。

它不是 Skyline Book 的教材庫，也不是 Skyline Runtime 的能力執行平台。

| 產品線 | 核心 | 目的 |
| --- | --- | --- |
| Skyline Book | Knowledge | 讓使用者理解 |
| Skyline Prompt | Skill | 讓使用者快速套用與共創 |
| Skyline Runtime | Execution | 讓事情真正發生 |
...
目前已實作完成下列套件：
- **地端主 Prompt 平台 (`packages/local-html`)**：獨立運行的 PWA/HTML 工具，為 Prompt 資產管理中心（SOP 範本設計、OCR 匯入、資料治理）。
- **Chrome 衛星側邊欄外掛 (`packages/chrome-extension`)**：定位為 PWA 的衛星工具，常駐於瀏覽器 Side Panel 隨手取用提示詞。支援匯入/匯出 JSON，以及強大的 **Live Tab 雙向同步**（免手動導出檔案，一鍵拉取/推送開啟中的地端 PWA 網頁資料），並能在 ChatGPT/Gemini/Claude 自動定位並填入對話框。

原始 Antigravity 原型保留在：

`/Users/force/Google_Antigravity/antigravity_start/skyline_prompt_hub`
...
`https://__Skyline_TAIWAN_IO__/prompt-demo/`

或明確開啟：

`https://__Skyline_TAIWAN_IO__/prompt-demo/index.html`
...
等 Prompt Manager 成熟後，再往 Skyline Runtime 演進。
...
- `docs/06_product_strategy_alignment.md`：Skyline Book / Prompt / Runtime 產品線對齊
...
- PWA 是下載與離線載體，Skyline 模型菜是內容與方法論
