# Skyline Product Strategy Alignment

本文用途：對齊 Skyline Prompt Manager 目前所屬產品線，避免太早把 Runtime / Capability Pack 的複雜度帶進 v0.3。
...
Skyline Prompt Manager 還在產品雛型階段，前期不需要太早處理：
...
- NotebookLM 幫人理解。
- GAS 負責分發。
- Skyline Prompt 管 Prompt / Workflow / Context。
- Skyline Runtime 才負責真正執行與能力調度。
...
- 強化搜尋、分類、標籤、狀態。
- 保持 PWA 可展示、可教學。
- 讓 README 能清楚說明本專案屬於 Skyline Prompt。
