# Skyline Prompt Manager v2.1 Release Notes
...
- **外掛端**：每 5 秒定時發送心跳探測 PING。僅當偵測到目標 PWA 分頁宣告了 `window.__Skyline_PWA_METADATA__` 屬性時，才進行握手判定，防範對一般 localhost 的誤判。
...
- 動態定義 `window.__Skyline_PWA_METADATA__` 機制，依據目前載入的 CSV 範本，即時回傳最新的 sourceName 與 headers 結構。

### 2.6 專屬備份腳本與版本打包工具
- 建立 `backup/backup_prompt_manager.py` Python 備份腳本。
- 一鍵執行會自動將專案（`skyline-prompt-manager`）打包為帶時間戳記與版本尾綴的 `.zip` 壓縮檔，並存放於 Git 忽略的 `backup/prompt-manager-backups/` 中，同時自動生成 `changelog_backup.txt` 變更說明檔放入壓縮檔內。
