# Skyline Prompt Manager v0.3 教材版概念對齊稿

本文用途：給 AI 與協作者快速理解 Skyline Prompt Manager v0.3 的設計方向。
...
Skyline Prompt Manager v0.3 是一個可拿來教學的 Prompt 管理器。

它不是一般 Prompt Library，而是把 Prompt 視為可管理、可教學、可備份、可轉換、可逐步接到 workflow 的「指令資產」。

在 Skyline 產品線中，這個專案屬於 Skyline Prompt。

它目前不是 Skyline Book，也不是 Skyline Runtime。
...
> Skyline 模型菜是內容與方法論，PWA 是讓使用者可下載、可安裝、可離線使用的載體。
...
Capability Pack / 功能卡概念保留，但它屬於未來 Skyline Runtime 藍圖，不主導 v0.3。
...
| 一般 Prompt Library | Skyline Prompt Manager v0.3 |
| --- | --- |
...
Skyline Prompt Manager 可以用「模型菜菜單」理解。
...
JSON Connect 是讓外部 AI 小工具和 Skyline Prompt Manager 對接的最小協議。

核心概念：

> 外部工具負責轉換，Skyline Prompt Manager 負責接收、整理、管理、備份、套用。
...
- 匯出目前資料
- 一鍵下載備份
- 檔名自動帶日期，例如 `skyline_prompt_backup_20260601.json`
- 還原內建範例
