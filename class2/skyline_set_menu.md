...
為幫助學員與企業主管（如G總、C董）直觀理解，我們將前述所有獨立模組（ETL、Prompt、RAG、PM）串接，形成一套完整的 **「Skyline 模型菜組合包 (Set Meal)」** 實戰範例：

```mermaid
flowchart LR
    A[🥗 前菜: ETL 轉運站<br>Skyline Mini Station] -->|1. 洗出 Markdown 文本| B[🍲 主菜: Prompt 平台<br>Prompt Manager]
    B -->|2. 拼裝 Prompt + 變數| C[🍛 湯品: 知識庫網關<br>AI_NotebookLM]
    C -->|3. 智慧分流與 RAG 檢索| D[🍨 甜點: 品質稽核與日誌<br>AI PM 系統]
```

## 🥗 前菜：資料前置清洗與 ETL (Skyline Mini Station)
* **功能**：同仁複製混亂的客戶投標需求（RFP），或直接截圖上傳。
* **輸出**：Mini Station 執行輕量級 OCR 與 ETL 清洗，自動過濾雜訊，產出結構乾淨的 Markdown 格式文本。

## 🍲 主菜：指令包裝與變數代入 (Skyline Prompt Manager)
...
* **衛星外掛 (Chrome Extension)**：為了加速日常工作流，我們同時提供了 **Skyline Prompt Manager 衛星外掛版 (Chrome Extension)**。該外掛直接承接地端主 Prompt 中心的資料庫，常駐於瀏覽器側邊欄 (Side Panel)，當同仁開啟 ChatGPT, Gemini, Claude 等 AI 網站時，可在側邊欄填入變數後「一鍵自動填入」對話框，免去手動複製貼上。同時在 v2.2 修復了選定不同卡片時置頂變數不更新之同步 Bug。
...
* **中心與衛星連動示意**：
  ![Skyline Prompt Manager 中心與衛星連動示意](reference/skyline-prompt-manager/docs/skyline_extension_mockup.png)
* **外掛與 NotebookLM 整合工作流示意**：
  ![Skyline Prompt Manager 搭配 NotebookLM 填充示意圖](reference/skyline-prompt-manager/docs/notebooklm_extension_flow.png)
* **v2.2 功能操作與同步示範錄影**：
  [skyline_prompt_manager_demo.mp4](reference/skyline-prompt-manager/docs/skyline_prompt_manager_demo.mp4)
...
* **專案連結**：詳細說明與一鍵模擬測試請見 **[AI 財會與稅務自動化專案](https://__Skyline_TAIWAN_IO__/ai-finance-demo1/)**。
